...
class ShopMenu:
    def __init__(self, player, toggle_menu):
        ...
        self.options = []
        self.menu_type = 'sell'
        self.sell_options = [item for item in SALE_PRICES.keys()]
        self.buy_options = [item for item in PURCHASE_PRICES.keys()]
    ...
    def switch_menu(self):
        """Switch between buy and sell menus and refresh options"""
        if self.menu_type == 'sell':
            self.menu_type = 'buy'
            # Update buy options to match current seed inventory
            self.buy_options = [item for item in PURCHASE_PRICES.keys() 
                              if item in self.player.seed_inventory]
            self.options = self.buy_options
        else:
            self.menu_type = 'sell'
            # Update sell options to match current item inventory
            self.sell_options = [item for item in SALE_PRICES.keys() 
    ...
    def input(self):
        ...
        if not self.timer.active:
            ...
            if keys[pygame.K_SPACE]:
                ...
                # sell
                if self.menu_type == 'sell':
                    if (current_item in SALE_PRICES and 
                        self.player.item_inventory[current_item] > 0):
                        # Reduce item inventory
                        self.player.item_inventory[current_item] -= 1
                        # Add money
                        self.player.money += SALE_PRICES[current_item]

                        # If selling a seed type item, also reduce seed inventory
                ...
                # buy
                    ...
                    price = PURCHASE_PRICES.get(current_item, 0)
                ...
                # Don't filter out zero quantity items anymore
                self.sell_options = [item for item in SALE_PRICES.keys()]
                self.buy_options = [item for item in PURCHASE_PRICES.keys()]
                
                # Keep current menu options
                if self.menu_type == 'sell':
                    self.options = self.sell_options
                else:
                    self.options = self.buy_options
    ...
    def show_entry(self, text_surf, amount, top, selected):
        ...
        # Get current item name for this entry
        current_item = self.options[self.text_surfs.index(text_surf)]

        # Get amount based on current menu type
        if self.menu_type == 'sell':
            amount = self.player.item_inventory.get(current_item, 0)
        else:
            amount = self.player.seed_inventory.get(current_item, 0)

        # Get price based on current menu type for this specific item
        if self.menu_type == 'sell':
            price = SALE_PRICES.get(current_item, 0)
        else:
            price = PURCHASE_PRICES.get(current_item, 0)

        # amount
        amount_surf = self.font.render(str(amount), False, 'Black')
        amount_rect = amount_surf.get_rect(midright = (self.main_rect.right - 80, bg_rect.centery))
        self.display_surface.blit(amount_surf, amount_rect)

        # price
        price_text = f"${price}"
        price_surf = self.font.render(price_text, False, 'Black')
